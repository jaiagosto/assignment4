""" 
This module is a calculator program that can do addition, subtraction, multiplication, and division
based on what the user enters. It uses Calculation classes for each operation
and also inclues extra features like help commands and 
history tracking to enhance user experience.

The code also shows two common programming paradigms in Python:
- LBYL (Look Before You Leap): This means checking if something is okay before doing it,
  like making sure the user input is valid before trying to use it.
- EAFP (Easier to Ask Forgiveness than Permission): This means trying to do something
  and handling any problems if they come up, like trying to divide and catching an error if
  the user tries to divide by zero.

Throughout the code, comments explain where and why these paradigms are used,
providing a comprehensive learning experience for us, students.
"""
